...
@livewire('page-breadcrumb', ['items' => [
    ...
    ['label' => __('relova.field_mappings')],
]])
<div class="mt-4 relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg overflow-hidden">
    {{-- Accent bar --}}
    <div class="h-1 bg-gradient-to-r from-sky-500 via-indigo-500 to-violet-500"></div>
    {{-- Header --}}
                                    ...
                                    <div wire:loading.remove wire:target="updatedTargetModule">
                                        <select wire:model="column_mappings.{{ $idx }}.local_field"
                                            ...
                                            @foreach($localColumns as $col)
                                                @if(!($col['primary'] ?? false) && !($col['foreign_key'] ?? false))
            ...
            <div class="flex items-center gap-2">
                ...
                @if($mappingUid)
                    <button wire:click="delete" type="button"
                        wire:loading.attr="disabled"
                        wire:target="delete"
                        wire:confirm="{{ __('relova.delete_mapping_confirm') }}"
                        class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 rounded-xl transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
                        <svg wire:loading.remove wire:target="delete" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                        </svg>
                        <svg wire:loading wire:target="delete" class="w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                        </svg>
                        <span wire:loading.remove wire:target="delete">{{ __('relova.delete_mapping') }}</span>
                        <span wire:loading wire:target="delete">{{ __('relova.loading') }}</span>
                    </button>
                @endif
            </div>
