...
<div class="mt-4 relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg overflow-hidden">
    ...
    <div class="p-5">
        ...
        @else
            {{-- List header --}}
            <div class="flex items-center gap-3 px-3.5 py-1.5 mb-1 select-none">
                <div class="flex-1 min-w-0 text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                    {{ __('relova.connection_host') }}
                </div>
                <div class="w-16 flex-shrink-0 text-center text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                    {{ __('relova.driver') }}
                </div>
                <div class="w-24 flex-shrink-0 text-right text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                    {{ __('relova.field_mappings') }}
                </div>
                <div class="w-32 flex-shrink-0 text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                    {{ __('relova.last_checked') }}
                </div>
                <div class="w-28 flex-shrink-0 text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                    {{ __('relova.status') }}
                </div>
            ...
            @foreach($connections as $conn)
                @php
                    ...
                    $pillBg = match(true) {
                        ! $connEnabled                => 'bg-zinc-100 text-zinc-500 dark:bg-slate-700/50 dark:text-slate-400 border-zinc-200 dark:border-slate-600',
                        $healthStatus === 'healthy'   => 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300 border-emerald-200 dark:border-emerald-700/40',
                        $healthStatus === 'degraded'  => 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300 border-amber-200 dark:border-amber-700/40',
                        $healthStatus === 'unhealthy' => 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-200 dark:border-red-700/40',
                        default                       => 'bg-zinc-100 text-zinc-500 dark:bg-slate-700/50 dark:text-slate-400 border-zinc-200 dark:border-slate-600',
                    };
                    ...
                    $lastCheck = isset($conn['last_health_check_at'])
                        ? \Carbon\Carbon::parse($conn['last_health_check_at'])->diffForHumans()
                        : '—';

                    $mappingCount = \Relova\Models\RelovaFieldMapping::where('connection_id', $conn['id'] ?? 0)->count();
                @endphp

                <div wire:key="conn-{{ $conn['uid'] }}"
                    style="border-left: 3px solid {{ $borderColor }}"
                    class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl border border-zinc-200 dark:border-slate-700/80 bg-white dark:bg-gray-800 mb-0.5 transition-colors hover:bg-zinc-50 dark:hover:bg-slate-700/60 {{ $connEnabled ? '' : 'opacity-60' }}">

                    {{-- Connection name + host --}}
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-1.5 flex-wrap">
                            <span class="text-sm font-semibold text-zinc-900 dark:text-white truncate">{{ $conn['name'] }}</span>
                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border border-purple-200 dark:border-purple-700/40 flex-shrink-0">
                                ⚡ BETA
                            </span>
                        </div>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            @if(! empty($conn['host']))
                                <span class="text-xs text-zinc-400 dark:text-slate-500 truncate font-mono">
                                    {{ $conn['host'] }}{{ ! empty($conn['port']) ? ':' . $conn['port'] : '' }}
                                </span>
                            @endif
                            @if($hasSsh)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400 border border-sky-200 dark:border-sky-700/40 flex-shrink-0">
                                    SSH
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="w-16 flex-shrink-0 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold {{ $driverBg }}">
                            {{ $driverLabel }}
                        </span>
                    </div>
                    ...
                    <div class="w-24 flex-shrink-0 text-right">
                        <span class="text-sm font-bold {{ $mappingCount > 0 ? 'text-zinc-700 dark:text-slate-300' : 'text-zinc-300 dark:text-slate-600' }}">
                            {{ $mappingCount }}
                        </span>
                        <span class="text-xs text-zinc-400 dark:text-slate-500 ml-0.5">{{ __('relova.fields_abbrev') }}</span>
                    </div>

                    {{-- Last health check --}}
                    <div class="w-32 flex-shrink-0 text-xs {{ $healthStatus === 'unhealthy' ? 'text-red-500 dark:text-red-400' : 'text-zinc-400 dark:text-slate-500' }}">
                        {{ $lastCheck }}
                    </div>

                    {{-- Status chip --}}
                    <div class="w-28 flex-shrink-0">
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-semibold border {{ $pillBg }}">
                    ...
                    {{-- Actions --}}
                    <div class="w-24 flex-shrink-0 flex items-center justify-end gap-0.5">
                        {{-- Toggle enabled --}}
                        <button wire:click="toggleConnection('{{ $conn['uid'] }}')" type="button"
                            class="p-1.5 rounded-lg transition-all duration-200 {{ $connEnabled ? 'text-emerald-500 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20' : 'text-zinc-400 dark:text-slate-500 hover:bg-zinc-100 dark:hover:bg-slate-700' }}"
                        ...
                        <button wire:click="openEditForm('{{ $conn['uid'] }}')" type="button"
                            class="p-1.5 text-zinc-400 dark:text-slate-500 hover:text-sky-600 dark:hover:text-sky-400 rounded-lg hover:bg-sky-50 dark:hover:bg-sky-900/20 transition-all duration-200"
                            title="{{ __('ui.edit') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                        ...
                        <button wire:click="deleteConnection('{{ $conn['uid'] }}')"
                            ...
                            class="p-1.5 text-zinc-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200"
                            title="{{ __('ui.delete') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            ...
            {{-- Add connection row --}}
            <button wire:click="openCreateForm" type="button"
                class="mt-2 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-dashed border-zinc-200 dark:border-slate-700 text-zinc-400 dark:text-slate-500 hover:border-teal-400 dark:hover:border-teal-600 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-teal-50/50 dark:hover:bg-teal-900/10 transition-all duration-200 text-sm font-medium">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
